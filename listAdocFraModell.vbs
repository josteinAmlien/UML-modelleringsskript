...
''	elementnavn = "«"&element.Stereotype&"» "&element.Name&""
	...
	if element.Abstract = 1 then
		elementnavn = adocKursiv( elementnavn & " (abstrakt)" )
	end if
	if innrykk = "=====" then
...
'''			Session.Output("|<<"&LCase(supplier.Name)&",«" & supplier.Stereotype&"» "&supplier.Name&">>")
			Session.Output("|<<"&LCase(supplier.Name) & "," & stereotypeNavn(supplier) & ">>")
...
''					Session.Output("|<<"&LCase(client.Name) & ",«" & client.Stereotype & "» " & client.Name & ">> +")
					Session.Output("|<<"&LCase(client.Name) & "," & stereotypeNavn(client) & ">> +")
				Else
''					Session.Output("<<"&LCase(client.Name) & ",«" & client.Stereotype & "» " & client.Name & ">> +")
					Session.Output("<<"&LCase(client.Name) & "," & stereotypeNavn(client) & ">> +")
				End If
...
function stereotypeNavn( element)
	stereotypeNavn = "«" & element.Stereotype & "» " & element.Name
end function
...
sub adocTabellOverskrift3( parameter, verdi, ekstra)
    Call adocTabellRad( adocBold(parameter), adocBold(verdi), adocBold(ekstra) )
end sub
...
''	
	Session.Output("[discrete]")
'''	Session.Output(innrykk & "== " & overskrift)  'Et lite HACK or å håndtere langt innrykk
	Session.Output(innrykk & "= " & overskrift)
end sub
