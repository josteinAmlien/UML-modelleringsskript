...
Sub ListAsciiDoc(innrykk,thePackage)
	...
	if thePackage.Element.Stereotype <> "" then
		overskrift = " Pakke: " & stereotype( thePackage.Element) & thePackage.Name
	else
		adocInsertPageBreak
		adocInsertBreak
		if innrykk = "=====" then
...
''		Session.Output("image::diagrammer/"&diag.Name&".png[link=diagrammer/"&diag.Name&".png, alt="""&alternativbildetekst&"""]")
		Session.Output("image::" & diagramFil & "[link=" & diagramFil & ", alt=""" & alternativbildetekst & """]")
...
'''						typ = stereo & typ
					...
					Session.Output("|<<" & LCase(typ) & "," & stereo & typ & ">>")
				else
					Session.Output("|" & typ & "")
				end if
			else
				Session.Output("|http://skjema.geonorge.no/SOSI/basistype/" & typ & "[" & typ & "]")		
			end if
...
' Supertype
	...
	For Each con In element.Connectors
		...
		If con.Type = "Generalization" And supplier.ElementID <> element.ElementID Then
			...
			Session.Output("|" + adocLink(supplier) )
...
'	Session.Output(innrykk&"= " & stereotypeNavn(element) & "")
...
function stereotypeNavn( element)
	stereotypeNavn = stereotype( element) & element.Name
end function
...
'		Session.Output(" -----DEBUG getPath=" & getPath & " package.Name = " & package.Name & " package.ParentID = " & package.ParentID & " package.Element.Stereotype = " & package.Element.Stereotype & " ----- ")
		if package.Element.Stereotype = "" then
			...
			path = stereotype( package.Element) & package.Name
		end if
